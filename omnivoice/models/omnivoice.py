...
def _resolve_model_path(name_or_path: str) -> str:
    ...
    # If it looks like a path but doesn't exist, don't try to download it as a repo ID
    if "/" in name_or_path and (name_or_path.startswith("/") or name_or_path.startswith("./")):
        raise FileNotFoundError(f"Model directory not found at: {name_or_path}. Please run the download script first.")
...
class OmniVoice(PreTrainedModel):
    ...
    def load_asr_model(self, model_name: str = "openai/whisper-large-v3-turbo"):
        ...
        # Resolve path and FORCE local loading
        model_name = _resolve_model_path(model_name)
        is_local = os.path.isdir(model_name)
        
        print(f"--- [Internal] Loading ASR from: {model_name} (Local: {is_local}) ---")
    ...
    def transcribe(
        ...
        # Force 16k for Whisper
        if sr != 16000:
            import torchaudio.functional as AF
            waveform_t = torch.from_numpy(waveform).float()
            if waveform_t.ndim == 1:
                waveform_t = waveform_t.unsqueeze(0)
            waveform = AF.resample(waveform_t, sr, 16000).squeeze(0).numpy()
            sr = 16000

        audio_input = {"array": waveform, "sampling_rate": sr}
        ts_arg = "word" if return_timestamps == "word" else return_timestamps
