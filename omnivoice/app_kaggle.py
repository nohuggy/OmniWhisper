...
def text_to_srt_whisper(text, audio_tuple, pipe, srt_max_words=17, language="zh", progress=None):
...
def generate_core(text, language, ref_audio, ref_text, instruct, num_step, guidance, denoise, speed, duration, pp, po, mode, srt_max_words=17, gen_srt=True, convert_punc=True, progress=gr.Progress()):
    """
    ...
    try:
        ...
        if gen_srt:
            ...
            def run_asr():
                try:
                    asr_res["content"] = text_to_srt_whisper(text, audio_tuple, pipe, srt_max_words=srt_max_words, progress=progress)
                except Exception as e:
                    asr_res["error"] = str(e)
...
def build_app(model_path=None, whisper_path=None):
    ...
    with gr.Blocks(theme=gr.themes.Soft(), css=CSS, title="OmniWhisper") as demo:
        ...
        def vc_handler(
            text, lang, ref, ref_text, instruct, steps, gs, dn, punc, speed, dur, pp, po, srt_max_words, gen_srt,
            ...
            for res in generate_core(
                ...
                pp=pp, po=po, mode="clone", srt_max_words=srt_max_words, gen_srt=gen_srt,
                progress=progress
            ):
                yield res
        ...
        def vd_handler(
            text, lang, speed, dur, steps, gs, dn, punc, po, srt_max_words, gen_srt, g1, g2, g3, g4, g5, g6,
            progress: gr.Progress = gr.Progress()
        ):
            if progress: progress(0, desc="🚀 Designing Voice...")
            instruct = ", ".join([g for g in [g1, g2, g3, g4, g5, g6] if g != "Auto"])
            for res in generate_core(text, lang, None, None, instruct, steps, gs, dn, speed, dur, False, po, "design", srt_max_words=srt_max_words, gen_srt=gen_srt, convert_punc=punc, progress=progress):
                yield res
