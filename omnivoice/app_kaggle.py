...
# ---------------------------------------------------------------------------
# 🚀 KAGGLE STABILITY & DEBUG ANNOTATION (IMMUTABLE GUARDRAILS)
# ---------------------------------------------------------------------------
# This edition solves the "Broken Connection" issue specific to Kaggle.
# 
...
# 
# ## 2. WHY LIGHTNING.AI WORKS BUT KAGGLE FAILED
# - SYSTEM OVERHEAD: Kaggle's Jupyter backend reserves ~800MB of VRAM for its UI 
...
def get_whisper_pipe(whisper_path=None):
    ...
    if WHISPER_PIPE is None:
        ...
        num_gpus = torch.cuda.device_count() if torch.cuda.is_available() else 0
        # If 2 GPUs, use cuda:1 for Whisper. Otherwise use cuda:0 or cpu.
        device = "cuda:1" if num_gpus >= 2 else ("cuda:0" if num_gpus == 1 else "cpu")
        dtype = torch.float32 if device == "cpu" else torch.float16
...
def text_to_srt_whisper(text, audio_tuple, pipe, srt_max_words=17, language="zh", asr_prompt="", progress=None):
    ...
    try:
        ...
        print(f"[SRT] Running smart_balanced_split (Target: 12, Max: {srt_max_words}) on {len(text)} chars...")
...
def generate_core(text, language, ref_audio, ref_text, instruct, num_step, guidance, denoise, speed, duration, pp, po, mode, asr_prompt="", srt_max_words=17, gen_srt=True, convert_punc=True, progress=gr.Progress()):
    """
    ...
    try:
        ...
        for curr, total, chunk_wave in gen_iter:
            if chunk_wave is None:
                ...
                # Yield every 1.5s to ensure Gradio/Kaggle proxy never times out
        ...
        yield audio_path, gr.update(), gr.update(), f"⏳ TTS Done ({len(full_waveform)/sr:.1f}s). Running ASR for alignment..."
        ...
        if gen_srt:
            # 🚀 RADICAL VRAM: Unload TTS before Whisper
            unload_tts()
            ...
            def run_asr():
                try:
                    asr_res["content"] = text_to_srt_whisper(text, audio_tuple, pipe, srt_max_words=srt_max_words, language=language, asr_prompt=asr_prompt, progress=progress)
                except Exception as e:
                    asr_res["error"] = str(e)
            ...
            while thread.is_alive():
                ...
                # HEARTBEAT: Constant yield to keep Kaggle/Gradio connection active
        ...
        status_msg = f"✅ Done in {elapsed:.1f}s | {len(full_waveform)/sr:.1f}s Audio | {tokens} Chars"
    ...
    with gr.Blocks(theme=gr.themes.Soft(), css=CSS, title="OmniWhisper") as demo:
        ...
        def vc_handler(
            ...
            for res in generate_core(
                ...
                srt_max_words=srt_max_words, progress=progress
            ):
                yield res
        ...
        def vd_handler(
            ...
            for res in generate_core(text, lang, None, None, instruct, steps, gs, dn, speed, dur, False, po, "design", asr_prompt=asr_prompt, gen_srt=gen_srt, convert_punc=punc, srt_max_words=srt_max_words, progress=progress):
                yield res
